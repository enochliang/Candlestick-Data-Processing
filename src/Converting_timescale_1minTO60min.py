...
def Converting_timescale_1minTO60min(imported,generated):
    ...
    for i in range(1,len(separatebyline)-1):
        ...
        if(counter==1):
            ...
            if(LOW_THIS<LOW):
                LOW=LOW_THIS
            if((counter==60)or((counter==45) and (separatebyline[i].split(',')[1]=='13:30:00') and (separatebyline[i+1].split(',')[1]!='13:31:00'))):
                if(counter==45):
...
if __name__=='__main__':
    Converting_timescale_1minTO60min(sys.argv[1],sys.argv[2])
